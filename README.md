...
2. `brew install tmux`. Every key that starts, shows or resumes an agent (`enter`, `v`, `n`, `N`, `R`) and `agtc attach` / `agtc send` need it. `brew install lazygit` is optional, `v` falls back to `git diff`. `brew install gh` for `P`.
...
| `V` | start a read-only reviewer agent for that session's work in a new tmux window. Asks for the spec (prefilled with the session's first prompt, `tab` for its last one, or type `@path/to/spec.md`) and which tool reviews (the other one by default). It shows as a row under the session. On a reviewer's row, `V` pastes its report into the reviewed session's input, unsent |
| `P` | push the checkout's branch and open its pull request in the browser, existing or new, in a popup that waits for enter. Refuses with uncommitted or untracked files, on the base branch, on a detached HEAD, or while a reviewer is still working |
...
- `P` runs `git push -u origin <branch> && { gh pr view --web || gh pr create --web; }` in a popup that closes on enter, after `git status --porcelain` came back empty and `git rev-parse --abbrev-ref HEAD` is neither `HEAD` nor the base branch.
...
- **Spawns on a key press only**: `pbcopy` (`c`), `tmux` pane commands (`enter`, `n`, `N`), the editor from `AGTC_EDITOR` (`o`), for `v` a tmux popup that runs `lazygit` or `git diff HEAD` in the checkout, for `P` a popup that runs `git push -u origin <branch>` and `gh pr view --web` / `gh pr create --web` after `git status` and `git rev-parse` in the checkout, and for `N` `git fetch` of the base branch plus `git worktree add` in the main checkout. `n` and `N` type the bare command `claude` or `codex` into a fresh shell in the checkout, nothing else; `R` and `S` type `claude --resume <id>` or `codex resume <id>` the same way. `V` types `claude "$(cat <prompt file>)" --session-id <uuid> --disallowedTools 'Edit,Write,NotebookEdit,Read(~/.claude/**),Read(~/.codex/**)' --allowedTools 'Read,Grep,Glob,Bash(git diff:*),…'` or `codex --sandbox read-only --ask-for-approval never "$(cat <prompt file>)"`. `agtc send` and `V` on a reviewer paste text into an agent's input without pressing enter (`V` reads the reviewer's last message from its transcript or rollout log first, and copies it with `pbcopy` when the pane is out of reach); `agtc attach` creates a grouped tmux session. Nothing in agtc removes a file, a branch or a worktree.
- **Network**: none while polling. `bun run check:offline` fails CI if anything under `src/` references fetch, http, sockets or Bun's server APIs. Two keys reach out, both by running other programs in front of you: `P` runs `git push` and `gh` in a tmux popup, and `agtc update` runs `bun add -g` (or `npm install -g`).
