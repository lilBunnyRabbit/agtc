...
`AGTC_WORKTREES`, `AGTC_BASE` and `AGTC_JUMP` are the environment forms of `--worktrees`, `--base` and `--jump`. `AGTC_TMUX_SETUP=0` stops agtc from binding `prefix a` / `option-a` and turning the mouse on in its tmux session. `AGTC_EDITOR` picks the editor for `o`. The default `zed` gets `--existing`: the checkout becomes its own workspace in the sidebar of the Zed window you already have open, or Zed switches to it if it is there already. This works regardless of the `cli_default_open_behavior` setting.
...
- **Spawns once at start inside tmux**: `tmux set-option mouse on` for its own session and `tmux bind-key` for `prefix a` and `option-a`, after `tmux list-keys` showed them free or already agtc's.
